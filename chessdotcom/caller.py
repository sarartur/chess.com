...
from chessdotcom.errors import ChessDotComResonser
...
class _internal:
    ...
    def do_get_request(cls, path):
        ...
        if r.status != 200:
            raise ChessDotComResonser(status_code=r.status)
        return r
...
def get_player_current_games(username: str):
    """
    :param username: username of the player.
    :returns: A ChessDotComResonser response object. The json property of the object
