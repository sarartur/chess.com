...
def get_current_daily_puzzle(
    tts=0, **request_options
) -> "GetCurrentDailyPuzzleResponse":
    """
    :param tts: the time the client will wait before making the first request.
    :returns: :obj:`GetCurrentDailyPuzzleResponse` object containing
                information about the daily puzzle found in www.chess.com.
    """
    return Resource(
        uri="/puzzle",
...
class ResponseBuilder(BaseResponseBuilder):
    def build(self, text):
        ...
        return GetCurrentDailyPuzzleResponse(
            json={"puzzle": data},
            text=text,
            puzzle=Puzzle(
...
class GetCurrentDailyPuzzleResponse(ChessDotComResponse):
    """
